...
namespace Kamgam.UIToolkitTextAnimation
{
    ...
    public class TextInfoAccessor
    {
        ...
        internal TextElementInfo[] m_textElementInfoArray;
        ...
        public void SourceTextIndexToRenderedIndex(int index, out int characterIndex, out int quadIndex)
        {
            ...
            m_textElementInfoArray = s_textElementInfoFieldMemberInfo.GetValue(textInfo) as TextElementInfo[];
            ...
            {
                var element = m_textElementInfoArray[i];
        ...
        public void GetCharacterAndTagInfos(List<AnimationTagInfo> animationTagInfos = null)
        {
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                var element = m_textElementInfoArray[i];
                var meshIndex = element.materialReferenceIndex;
                var vertexIndex = element.vertexIndex;
        ...
        public bool MeshIsDirty()
        {
            ...
            int previousHash = uitkTextHandle.m_PreviousGenerationSettingsHash;
#if UNITY_6000 || UNITY_6000_1_OR_NEWER
            bool isDirty = uitkTextHandle.IsDirty(previousHash);
#else
            bool isDirty = true; // In Unity 2023 this does not exist thus we assume it's true at all time.
#endif
        ...
        internal List<TextCoreVertex[]> m_tmpMeshVertices = new();
        ...
        public void CacheMeshVertexAccess()
        {
            ...
            // Would be nice to cache these too but sadly the textElementInfo is regenerated internally
            // so we have to reacquire the reference every time.
            var uitkTextHandle = GetUITKTextHandle() as TextHandle;
            if (uitkTextHandle == null)
                return;
            
            var meshInfos = uitkTextHandle.textInfo.meshInfo;
            if (meshInfos != null)
            {
                ...
                {
                    var vertices = meshInfos[i].vertexData;
        ...
        public void ExecuteOnAllVertices(ChangeCharacterDelegate changeCharacterFunc)
        {
            ...
            for (int c = 0; c < charCount; c++)
            {
                ...
                {
                    // Cache
                    position0 = vertices[vertexIndex0].position;
                    position1 = vertices[vertexIndex1].position;;
                    position2 = vertices[vertexIndex2].position;;
                    position3 = vertices[vertexIndex3].position;;
                    
                    color0 = vertices[vertexIndex0].color;
                    color1 = vertices[vertexIndex0].color;
                    color2 = vertices[vertexIndex0].color;
                    color3 = vertices[vertexIndex0].color;
                ...
                if (validPositionCount == 4)
                {
                    ...
                    // Write Position
                    var vertex = vertices[vertexIndex0];
                    vertex.position = quadVertexData.BottomLeftPosition;
                    vertices[vertexIndex0] = vertex;
                    
                    vertex = vertices[vertexIndex1];
                    vertex.position = quadVertexData.TopLeftPosition;
                    vertices[vertexIndex1] = vertex;
                    
                    vertex = vertices[vertexIndex2];
                    vertex.position = quadVertexData.TopRightPosition;
                    vertices[vertexIndex2] = vertex;
                    
                    vertex = vertices[vertexIndex3];
                    vertex.position = quadVertexData.BottomRightPosition;
                    vertices[vertexIndex3] = vertex;

                    // Write Color
                    vertex = vertices[vertexIndex0];
                    vertex.color = quadVertexData.BottomLeftColor;
                    vertices[vertexIndex0] = vertex;
                    
                    vertex = vertices[vertexIndex1];
                    vertex.color = quadVertexData.TopLeftColor;
                    vertices[vertexIndex1] = vertex;
                    
                    vertex = vertices[vertexIndex2];
                    vertex.color = quadVertexData.TopRightColor;
                    vertices[vertexIndex2] = vertex;
                    
                    vertex = vertices[vertexIndex3];
                    vertex.color = quadVertexData.BottomRightColor;
                    vertices[vertexIndex3] = vertex;
